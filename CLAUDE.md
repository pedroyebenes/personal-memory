...
5. Generate embeddings per chunk (`app/processing/embeddings.py` — sentence-transformer or hash fallback)
6. Write to SQLite FTS5 + embeddings tables
...
- **SQLite is authoritative**: `app/schema.sql` defines 9 tables (including FTS5 virtual table); `app/db.py` manages connection
